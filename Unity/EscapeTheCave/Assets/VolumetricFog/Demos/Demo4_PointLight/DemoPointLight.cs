...
namespace VolumetricFogAndMist {
    public class DemoPointLight : MonoBehaviour {
        ...
        Vector3[] fairyDirections;

        private void Start() {
            fairyDirections = new Vector3[VolumetricFog.MAX_POINT_LIGHTS];
            for (int k = 0; k < fairyDirections.Length; k++) {
                Light fairy = VolumetricFog.instance.GetPointLight(k);
                if (fairy != null) {
                    fairy.color = new Color(Random.value, Random.value, Random.value);
                }
            }
        }
